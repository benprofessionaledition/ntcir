...
def masked_conv1d_and_max(t, weights, filters, kernel_size):
    ...
    with tf.variable_scope("masked_1d_conv"):
        ...
        shape = tf.shape(t) # [batch size, number of words, number of chars per word, char embedding size] == "[? ? ? 100]"
        ndims = t.shape.ndims
        dim1 = reduce(lambda x, y: x*y, [shape[i] for i in range(ndims - 2)]) # flatten
        dim2 = shape[-2]
        dim3 = t.shape[-1]
